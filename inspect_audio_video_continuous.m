...
switch exp_type
    ...
    case {'adult','adult_social'}
        ...
        call_info = struct('eventpos',num2cell(vertcat(event_pos_data.file_event_pos),2),...
            'corrected_eventpos',num2cell(vertcat(event_pos_data.corrected_eventpos),2),...
            'batsInvolved',[],'instigator',[],'facingCaller',[],'notFacingCaller',[],...
            'behaviors',repmat({cell(1,nBehaviors)},length(event_pos_data),1),...
...
vocalizationPanel = uipanel(params.hFig,'unit','normalized','Title','Vocalization Panel',...
    'Position',[0.02 0.35 0.076 0.55],'Tag','vocalization');
...
uicontrol(vocalizationPanel,'unit','normalized','style','listbox','string',...
        [{''} params.bat_IDs],'position',[0.01 0.75 0.9 0.175],'value',involvedValue,...
...
uicontrol(vocalizationPanel,'unit','normalized','style','text','string',...
    'Bats Involved','position',[0.01 0.925 0.9 0.025]);
    
instigatorValue = get_bats_involved(call_info,call_k,params,'instigator');

uicontrol(vocalizationPanel,'unit','normalized','style','listbox','string',...
        [{''} params.bat_IDs],'position',[0.01 0.525 0.9 0.175],'value',instigatorValue,...
        'Min',0,'Max',length(params.bat_IDs),'callback',{@updateCallInfoCallback,params,call_k},...
        'Tag','instigator');

uicontrol(vocalizationPanel,'unit','normalized','style','text','string',...
    'Instigator','position',[0.01 0.7 0.9 0.025]);

facingValue = get_bats_involved(call_info,call_k,params,'facingCaller');

uicontrol(vocalizationPanel,'unit','normalized','style','listbox','string',...
        [{''} params.bat_IDs],'position',[0.01 0.3 0.9 0.175],'value',facingValue,...
        'Min',0,'Max',length(params.bat_IDs),'callback',{@updateCallInfoCallback,params,call_k},...
        'Tag','facingCaller');

uicontrol(vocalizationPanel,'unit','normalized','style','text','string',...
    'Facing caller?','position',[0.01 0.475 0.9 0.025]);
    
nonFacingValue = get_bats_involved(call_info,call_k,params,'notFacingCaller');

uicontrol(vocalizationPanel,'unit','normalized','style','listbox','string',...
        [{''} params.bat_IDs],'position',[0.01 0.05 0.9 0.175],'value',nonFacingValue,...
        'Min',0,'Max',length(params.bat_IDs),'callback',{@updateCallInfoCallback,params,call_k},...
        'Tag','notFacingCaller');

uicontrol(vocalizationPanel,'unit','normalized','style','text','string',...
     'Not facing caller?','position',[0.01 0.225 0.9 0.025]);
...
uicontrol(vocalizationPanel,'unit','normalized','style','text','string',...
    ['Vocalizer: ' voc_bat_ID],'position',[0.01 0 0.9 0.04]);
...
uicontrol(params.hFig,'unit','normalized','style','popupmenu','string',...
    ...
    'position',[0.02,0.25,0.05,0.05],'Tag','loadNextAudioFile',...
